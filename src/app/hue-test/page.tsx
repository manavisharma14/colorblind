...
import { colorBoxes, ColorBox, ColorChip, shuffleBox } from "../../data/colors";
import Results from "./Results";

export default function HueTest() {
  const [currentBoxIndex, setCurrentBoxIndex] = useState(0);

  // Initialize boxes with shuffled middle chips, fixed ends
    ...
    <div key={currentBoxIndex} className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="max-w-4xl mx-auto">
        <p className="text-center mb-6">
          Arrange the colors in order by hue. Drag the chips to reorder.
        </p>
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-semibold mb-4">
            Box {box.id}: {box.title}
          </h2>
          <div className="flex gap-4 justify-center p-4 bg-gray-100 rounded">
            {box.chips.map((chip: ColorChip, index: number) => {
              const isFixed = index === 0 || index === box.chips.length - 1;

              return (
                <div
                  key={chip.id}
                  draggable={!isFixed}
                  onDragStart={(e) => !isFixed && handleDragStart(e, index)}
                  onDragOver={(e) => !isFixed && handleDragOver(e)}
                  onDrop={(e) => !isFixed && handleDrop(e, index)}
                  className={`w-16 h-16 rounded-full border-2 flex items-center justify-center transition-transform
                    ${
                      isFixed
                        ? "cursor-not-allowed border-gray-500"
                        : "cursor-move hover:scale-110 border-gray-300"
                    }`}
